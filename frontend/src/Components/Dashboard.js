...
export default function Dashboard() {
  ...
  // Add this state near the top with other state declarations
  const [editingUnifiedStatusCell, setEditingUnifiedStatusCell] = useState(null); // {specId, rowIndex}
  const [unifiedStatusChanges, setUnifiedStatusChanges] = useState({});
  ...
  const [updateReportMessage, setUpdateReportMessage] = useState(null);
  const [updateReportError, setUpdateReportError] = useState(null);
  const [saveFinalReportLoading, setSaveFinalReportLoading] = useState(false);
  const [saveFinalReportMessage, setSaveFinalReportMessage] = useState(null);
  const [saveFinalReportError, setSaveFinalReportError] = useState(null);
  ...
  useEffect(() => {
    if (activeStep === 3 && apiResponse) {
      ...
      Object.entries(dataToProcess).forEach(([specId, specData]) => {
  ...
  useEffect(() => {
    const fetchHistoryData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(result.error || result.errorMessage || result.errorType || 'Failed to fetch history data');
        }
        
        // Check for API error status
        if (result.err_status === true) {
          setHistoryError(result.err_message || 'Error loading history data');
          setHistoryData([]);
          return;
        }

        if (result.data && result.data.history_response) {
          setHistoryData(result.data.history_response);
        } else {
          setHistoryData([]);
        }
        
        if (result.data && result.data.total_counts) {
          setTotalCounts(result.data.total_counts);
        }
  ...
  useEffect(() => {
    ...
    const finalReportData = localStorage.getItem('finalReportData');

    if (mtrResponse) {
      const parsedResponse = JSON.parse(mtrResponse);
      
      // If coming from history with final report data
      if (activeStep === '6' && finalReportData) {
        // Parse finalReportData and set it as apiResponse
        const parsedFinalReport = JSON.parse(finalReportData);
        setApiResponse({ data: parsedFinalReport });
      } else {
        // Regular flow
    ...
    <Box sx={{ 
      ...
      {addmtr === false ? (
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    if (row.status === 'review') {
                                      ...
                                      if (data.err_status === true) {
                                        setRecordViewError(data.err_message || 'Failed to fetch final report');
                                        setShowRecordViewError(true);
                                        return;
                                      }
                                      
                                      // Enhance the response with record_id before setting it in the state
                                      const enhancedData = {
                                        ...data,
                                        data: {
                                          ...data.data,
                                          record_id: row.record_id // Add record_id to the data object
                                        }
                                      };
                                      
                                      // Store the response in localStorage for FinalReportTable
                                      localStorage.setItem('finalReportData', JSON.stringify(enhancedData));
                                      // Store the MTR ID for later use
                                      localStorage.setItem('mtrId', row.record_id);
                                      
                                      // Navigate to step 7 (Final Report Review)
                                      setApiResponse(enhancedData);
                                      setaddMtr(true);
                                      setActiveStep(6);
                                      return;
                                    }
                                    ...
                                    if (!response.ok || data.err_status === true) {
                                      throw new Error(data.err_message || data.error || data.errorMessage || data.errorType || 'Failed to fetch record view data');
                                    }
                                    ...
                                    const formattedResponse = {
                                      decision: {
                                        heat_number: data.data?.decision?.heat_number || data.decision?.heat_number || '',
                                        lot_number: data.data?.decision?.lot_number || data.decision?.lot_number || '',
                                        raw_material_size: data.data?.decision?.raw_material_size || data.decision?.raw_material_size || '',
                                        strength: data.data?.decision?.strength || data.decision?.strength || '',
                                        type: data.data?.decision?.type || data.decision?.type || ''
                                      },
                                      mds_name: data.data?.mds_name || data.mds_name || '',
                                      pdf_text: data.data?.pdf_text || data.pdf_text || '',
                                      record_id: row.record_id // Add the record_id to the formatted response
                                    };
                                    
                                    // Set the formatted response and navigate to step 2
                                    setApiResponse(formattedResponse);
                                    setStep1Response(formattedResponse);
                                    setMdsName(data.data?.mds_name || data.mds_name || '');
                                    setTypeValue(data.data?.decision?.type || data.decision?.type || '');
                                    setaddMtr(true);
          ...
          <Box sx={{ width: '100%', maxWidth: 1100, mx: 'auto'}}>
              ...
              <>
                ...
                {activeStep === 2 && (
                  <Paper elevation={0} sx={{
                    ...
                    {/* Error Alert if any */}
                    {apiResponse && apiResponse.err_status === true ? (
                      <Alert 
                        severity="error" 
                        sx={{ 
                          mb: 2,
                      ...
                      >
                        {apiResponse.err_message || 'An error occurred while processing the request.'}
                      </Alert>
                      ...
                      >
                        ...
                        {(mtrMode === 'data' || mtrMode === 'data+pdf') && apiResponse && (
                          // First try the direct structure that matches the API response
                          (apiResponse.data && apiResponse.data.extracted_mtr_keys && apiResponse.data.extracted_mtr_keys.response) ? (
                            <Box
                              sx={{
                                flex: mtrMode === 'data+pdf' ? 1 : 'unset',
                                width: mtrMode === 'data+pdf' ? { xs: '100%', md: '50%' } : '100%',
                            ...
                            >
                              <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', borderRadius: 8, overflow: 'hidden' }}>
                                ...
                                <tbody>
                                  {Object.entries(apiResponse.data.extracted_mtr_keys.response).flatMap(([category, fields]) =>
                            ...
                            // Fallback for other possible structures (like when extracted_mtr_keys is at the top level)
                            apiResponse.extracted_mtr_keys && apiResponse.extracted_mtr_keys.response && (
                              <Box
                                sx={{
                                  flex: mtrMode === 'data+pdf' ? 1 : 'unset',
                                  width: mtrMode === 'data+pdf' ? { xs: '100%', md: '50%' } : '100%',
                    ...
                    {apiResponse && apiResponse.err_status === true ? (
                      ...
                      >
                        ...
                        {(mtrMode === 'data' || mtrMode === 'data+pdf') && apiResponse && (
                          ...
                          >
                            {Object.entries(apiResponse.data || apiResponse).map(([specId, specData]) => {
                                    ...
                                    <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', borderRadius: 8, overflow: 'hidden' }}>
                                      ...
                                      <tbody>
                                        {resultsArr.map((row, idx) => {
                                          ...
                                          const isEditingStatus = editingUnifiedStatusCell && editingUnifiedStatusCell.specId === specId && editingUnifiedStatusCell.rowIndex === idx;
                                          ...
                                          // Get current status value (from changes or original)
                                          const statusKey = `${specId}-${idx}`;
                                          const currentStatus = unifiedStatusChanges[statusKey] || row.status;
                                          ...
                                          return (
                                            <tr key={row.checkpoint + '-' + idx} className="border-t border-gray-200">
                                              ...
                                              <td 
                                                ...
                                                onClick={() => setEditingUnifiedStatusCell({ specId, rowIndex: idx })}
                                              >
                                                {isEditingStatus ? (
                                                  <select
                                                    value={currentStatus}
                                                    autoFocus
                                                    onChange={e => {
                                                      setUnifiedStatusChanges(prev => ({ ...prev, [statusKey]: e.target.value }));
                                                      setEditingUnifiedStatusCell(null);
                                                    }}
                                                    onBlur={() => setEditingUnifiedStatusCell(null)}
                                                    style={{
                                                      padding: '4px 8px',
                                                      borderRadius: '16px',
                                                      border: '1px solid',
                                                      ...
                                                      ...currentStatus === 'Pass' ? {
                                                        ...
                                                        background: '#f5f5f5', color: '#5f6368', borderColor: '#e0e0e0'
                                                      }
                                                    }}
                                                  >
                                                  ...
                                                  <span style={{
                                                    ...
                                                    {currentStatus === 'Pass' && <span style={{ fontSize: 16, marginRight: 6 }}>✅</span>}
                                                    {currentStatus === 'Fail' && <span style={{ fontSize: 16, marginRight: 6 }}>❌</span>}
                                                    {currentStatus === 'Review' && <span style={{ fontSize: 16, marginRight: 6 }}>⚠️</span>}
                                                    {currentStatus}
                                                  </span>
                                                )}
                                              </td>
                ...
                {activeStep === 6 && (
                  <Paper elevation={0} sx={{
                    ...
                    {/* Error alert or table based on err_status */}
                    {apiResponse && apiResponse.err_status === true ? (
                      <Alert 
                        severity="error" 
                        sx={{ 
                      ...
                      >
                        {apiResponse.err_message || 'An error occurred while fetching the final report.'}
                      </Alert>
                    ) : !apiResponse || !apiResponse.data || !apiResponse.data.openai_final_report || 
                        (Array.isArray(apiResponse.data.openai_final_report) && apiResponse.data.openai_final_report.length === 0) ? (
                      <Alert 
                        severity="info" 
                        sx={{ 
                          m: 2,
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', borderRadius: 8, overflow: 'hidden' }}>
                          <thead style={{ background: '#f3f4f6' }}>
                            <tr>
                              ...
                              <th style={{ textAlign: 'left', padding: '12px 16px', fontWeight: 600, fontSize: 15, color: '#444' }}>Unified</th>
                              <th style={{ textAlign: 'left', padding: '12px 16px', fontWeight: 600, fontSize: 15, color: '#444' }}>Status</th>
                              <th style={{ textAlign: 'left', padding: '12px 16px', fontWeight: 600, fontSize: 15, color: '#444' }}>Actions</th>
                            </tr>
                          </thead>
                          <tbody>
                          {apiResponse && apiResponse.data && apiResponse.data.openai_final_report && (
                              Array.isArray(apiResponse.data.openai_final_report) ? apiResponse.data.openai_final_report : [apiResponse.data.openai_final_report]
                            ).map((item, index) => {
                              // Check if this row has been edited
                              const isRowEdited = cellChanges[`${index}-checkpoint`] !== undefined || 
                                                 cellChanges[`${index}-category`] !== undefined || 
                                                 cellChanges[`${index}-field`] !== undefined || 
                                                 cellChanges[`${index}-mtr`] !== undefined || 
                                                 cellChanges[`${index}-unified`] !== undefined ||
                                                 statusChanges[index] !== undefined ||
                              ...
                              <tr key={index} className="border-t border-gray-200">
                                <td 
                                  style={{ padding: '12px 16px', fontSize: 14, color: '#444', cursor: 'pointer' }}
                                  onClick={() => setEditingCell({ index, field: 'checkpoint' })}
                                >
                                  {editingCell && editingCell.index === index && editingCell.field === 'checkpoint' ? (
                                    <input
                                      type="text"
                                      value={cellChanges[`${index}-checkpoint`] !== undefined ? cellChanges[`${index}-checkpoint`] : item.checkpoint}
                                      autoFocus
                                      style={{ fontSize: 14, width: '100%', padding: 4 }}
                                      onChange={e => {
                                        setCellChanges(prev => ({
                                          ...prev,
                                          [`${index}-checkpoint`]: e.target.value
                                        }));
                                      }}
                                    ...
                                    cellChanges[`${index}-checkpoint`] || item.checkpoint
                                  )}
                                </td>
                                <td 
                                  style={{ padding: '12px 16px', fontSize: 14, color: '#444', cursor: 'pointer' }}
                                  onClick={() => setEditingCell({ index, field: 'category' })}
                                >
                                  {editingCell && editingCell.index === index && editingCell.field === 'category' ? (
                                    <input
                                      type="text"
                                      value={cellChanges[`${index}-category`] !== undefined ? cellChanges[`${index}-category`] : item.category}
                                      autoFocus
                                      style={{ fontSize: 14, width: '100%', padding: 4 }}
                                      onChange={e => {
                                        setCellChanges(prev => ({
                                    ...
                                    cellChanges[`${index}-category`] || item.category
                                  )}
                                </td>
                                <td 
                                  style={{ padding: '12px 16px', fontSize: 14, color: '#444', cursor: 'pointer' }}
                                  onClick={() => setEditingCell({ index, field: 'field' })}
                                >
                                  {editingCell && editingCell.index === index && editingCell.field === 'field' ? (
                                    <input
                                      type="text"
                                      value={cellChanges[`${index}-field`] !== undefined ? cellChanges[`${index}-field`] : item.field}
                                      autoFocus
                                      style={{ fontSize: 14, width: '100%', padding: 4 }}
                                      onChange={e => {
                                        setCellChanges(prev => ({
                                    ...
                                    cellChanges[`${index}-field`] || item.field
                                  )}
                                </td>
                                <td 
                                  style={{ padding: '12px 16px', fontSize: 14, color: '#444', cursor: 'pointer' }}
                                  onClick={() => setEditingCell({ index, field: 'mtr' })}
                                >
                                  {editingCell && editingCell.index === index && editingCell.field === 'mtr' ? (
                                    <input
                                      type="text"
                                      value={cellChanges[`${index}-mtr`] !== undefined ? cellChanges[`${index}-mtr`] : item.MTR}
                                      autoFocus
                                      style={{ fontSize: 14, width: '100%', padding: 4 }}
                                      onChange={e => {
                                        setCellChanges(prev => ({
                                          ...prev,
                                          [`${index}-mtr`]: e.target.value
                                        }));
                                      }}
                                      onBlur={() => setEditingCell(null)}
                                      onKeyDown={e => {
                                    ...
                                    cellChanges[`${index}-mtr`] || item.MTR
                                  )}
                                </td>
                                <td 
                                  style={{ padding: '12px 16px', fontSize: 14, color: '#444', cursor: 'pointer' }}
                                  onClick={() => setEditingCell({ index, field: 'unified' })}
                                >
                                  {editingCell && editingCell.index === index && editingCell.field === 'unified' ? (
                                    <input
                                      type="text"
                                      value={cellChanges[`${index}-unified`] !== undefined ? cellChanges[`${index}-unified`] : item.Unified}
                                      autoFocus
                                      style={{ fontSize: 14, width: '100%', padding: 4 }}
                                      onChange={e => {
                                        setCellChanges(prev => ({
                                          ...prev,
                                          [`${index}-unified`]: e.target.value
                                        }));
                                      }}
                                      onBlur={() => setEditingCell(null)}
                                      onKeyDown={e => {
                                    ...
                                    cellChanges[`${index}-unified`] || item.Unified
                                  )}
                                </td>
                                ...
                                <td style={{ padding: '12px 16px', fontSize: 14 }}>
                                  {isRowEdited && (
                                    <Button
                                      variant="contained"
                                      size="small"
                                      disabled={updateReportLoading[index]}
                                      sx={{
                                        bgcolor: '#4285f4',
                                        color: '#fff',
                                        fontSize: 12,
                                      ...
                                      onClick={async () => {
                                        try {
                                          // Get row_id and report_id from the item
                                          const row_id = item.row_id || index + 1; // Fallback to index+1 if no row_id
                                          const report_id = item.report_id || apiResponse.data?.report_id; // Get report_id from response
                                          
                                          // Get MTR, Unified, and Status values (use edited values if available)
                                          const mtrValue = cellChanges[`${index}-mtr`] !== undefined ? cellChanges[`${index}-mtr`] : item.MTR;
                                          const unifiedValue = cellChanges[`${index}-unified`] !== undefined ? cellChanges[`${index}-unified`] : item.Unified;
                                          const statusValue = statusChanges[index] || item.Status;
                                          ...
                                          setUpdateReportLoading(prev => ({ ...prev, [index]: true }));
                                          
                                          // Call the API
                                          const response = await fetch('/api/proxy/update-report', {
                                            method: 'PUT',
                                            headers: {
                                          ...
                                          const result = await response.json();
                                          console.log('Update report API response:', result);
                                          
                                          // Handle the API response
                                          if (result.err_status === true) {
                                            // Show error message
                                            setUpdateReportError(result.err_message || 'Failed to update report');
                                            console.error('API error:', result.err_message);
                                          } else {
                                            // Show success message
                                            setUpdateReportMessage(result.data?.message || 'Changes saved successfully');
                                            
                                            // Clear editing state for this row
                                            if (editingCell?.index === index) {
                                              setEditingCell(null);
                                            }
                                            if (editingStatusRow === index) {
                                              setEditingStatusRow(null);
                                            }
                                          }
                                        } catch (error) {
                                          console.error('Error updating report:', error);
                                          setUpdateReportError(error.message || 'An error occurred while updating the report');
                                        } finally {
                                          // Clear loading state for this row
                                          setUpdateReportLoading(prev => ({ ...prev, [index]: false }));
                                        }
                                      }}
                                    >
                                      {updateReportLoading[index] ? (
                                        <CircularProgress size={16} color="inherit" sx={{ mx: 0.5 }} />
                                      ) : 'Save'}
                                    </Button>
                                  )}
                                </td>
                ...
                {activeStep !== 6 ? (
                        ...
                        onClick={() => {
                          if (activeStep === 0) {
                            ...
                            .then(data => {
                              ...
                              if (data.error || data.detail === 'Not Found') {
                                throw new Error(data.error || 'API returned Not Found error');
                              }
                              
                              // Check if API returned an error status
                              if (data.err_status === true) {
                                console.error('API returned error:', data.err_message);
                                throw new Error(data.err_message || 'API returned an error');
                              }
                              
                              // Check for the proper data structure
                              if (!data.data || !data.data.decision) {
                                console.warn('Unexpected API response structure:', data);
                                // If the response is flattened directly
                                if (data.decision) {
                                ...
                                console.log('Type from API:', data.data.decision.type);
                                setApiResponse(data.data);
                                setStep1Response(data.data);
                                setTypeValue(data.data.decision.type || '');
                              }
                            ...
                            .catch(error => {
                              ...
                              setApiResponse({ error: error.message || 'Failed to process file' });
                              setApiError(error.message || 'Failed to process file');
                              setShowApiError(true);
                              setLoading(false);
                            });
                            ...
                            const formData = new FormData();
                            // Use the record_id from apiResponse, which now contains it when loaded from history
                            formData.append('pdf_text_id', apiResponse?.record_id || '');

                            console.log('Sending pdf_text_id to extract key API:', apiResponse?.record_id || '');
                            ...
                            .then(data => {
                              ...
                              // Make sure to preserve the record_id in the response
                              const enhancedData = {
                                ...data,
                                record_id: apiResponse?.record_id // Ensure record_id is preserved
                              };
                              
                              setApiResponse(enhancedData);
                              setStep2Response(enhancedData);
                              if (data?.material_identification?.material_grade) {
                                setMaterialType(data.material_identification.material_grade);
                              }
                              if (data?.data?.mtr_keys_id) {
                                setMtrKeysId(data.data.mtr_keys_id);
                              } else if (data?.mtr_keys_id) {
                            ...
                            .catch(error => {
                              ...
                              setApiResponse({ error: error.message || 'Failed to extract keys' });
                              setApiError(error.message || 'Failed to extract keys');
                              setShowApiError(true);
                              setLoading(false);
                            });
                            ...
                            .catch(error => {
                              ...
                              setApiResponse({ error: error.message || 'Failed to get unified output' });
                              setApiError(error.message || 'Failed to get unified output');
                              setShowApiError(true);
                              setLoading(false);
                            });
                          } else if (activeStep === 3) {
                            ...
                            .catch(error => {
                              ...
                              setApiResponse({ error: error.message || 'Failed to generate final report' });
                              setApiError(error.message || 'Failed to generate final report');
                              setShowApiError(true);
                              setLoading(false);
                            });
                          } else {
                  ...
                  <Box sx={{ position: 'relative', minHeight: 80, mt: 6, mb:4 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 2, position: 'absolute', right: 0, bottom: 0 }}>
                      ...
                      <Button
                        variant="contained"
                        disabled={saveFinalReportLoading}
                        sx={{
                          bgcolor: '#4285f4',
                          color: '#fff',
                          fontWeight: 600,
                        ...
                        onClick={async () => {
                          try {
                            // Get record_id from API response with improved path navigation
                            const recordId = apiResponse?.data?.record_id || 
                                             apiResponse?.record_id || 
                                             localStorage.getItem('mtrId');
                            
                            if (!recordId) {
                              setSaveFinalReportError('Record ID not found');
                              console.error('Record ID not found in API response or localStorage');
                              return;
                            }
                            
                            // Log the record_id
                            console.log('Save final report with record_id:', recordId);
                            
                            // Set loading state
                            setSaveFinalReportLoading(true);
                            ...
                            // Call the API
                            const response = await fetch('/api/proxy/savefinalreport', {
                              method: 'PUT',
                              body: formData
                            });
                            
                            const result = await response.json();
                            console.log('Save final report API response:', result);
                            
                            // Handle the API response
                            if (result.err_status === true) {
                              // Show error message
                              setSaveFinalReportError(result.err_message || 'Failed to save final report');
                              console.error('API error:', result.err_message);
                            } else {
                              // Show success message
                              setSaveFinalReportMessage(result.data?.message || 'Final report saved successfully');
                              
                              // After successful save, navigate back to the dashboard
                              setTimeout(() => {
                                setaddMtr(false);
                                // Reset all states when going back to dashboard
                                setApiResponse(null);
                            ...
                            setSaveFinalReportError(error.message || 'An error occurred while saving the final report');
                          } finally {
                            // Clear loading state
                            setSaveFinalReportLoading(false);
                          }
                        }}
                      >
                        {saveFinalReportLoading ? (
            ...
            {/* Add success and error Snackbars for update report */}
            <Snackbar
              open={!!updateReportMessage}
              autoHideDuration={3000}
              onClose={() => setUpdateReportMessage(null)}
              anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
            >
              <Alert 
                severity="success" 
            ...
            <Snackbar
              open={!!updateReportError}
              autoHideDuration={3000}
              onClose={() => setUpdateReportError(null)}
              anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
            >
              <Alert 
                severity="error" 
            ...
            {/* Add success and error Snackbars for save final report */}
            <Snackbar
              open={!!saveFinalReportMessage}
              autoHideDuration={3000}
              onClose={() => setSaveFinalReportMessage(null)}
              anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
            >
              <Alert 
                severity="success" 
                ...
                sx={{ width: '100%' }}
              >
                {saveFinalReportMessage}
              </Alert>
            </Snackbar>

            <Snackbar
              open={!!saveFinalReportError}
              autoHideDuration={3000}
              onClose={() => setSaveFinalReportError(null)}
              anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
            >
              <Alert 
                severity="error" 
